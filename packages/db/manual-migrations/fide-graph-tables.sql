...
--
-- 1. reference_identifiers: Lookup table mapping fingerprints ↔ reference identifiers
...
-- 1. REFERENCE IDENTIFIERS (Fingerprint → reference identifier Lookup)
-- ============================================================================
-- Maps Fide ID fingerprints back to their reference identifiers (human-readable form).
...
--   reference_identifier: The reference string (e.g., "x.com/alice", "schema:worksFor", "@alice") - NOT NULL
--
-- NOTE: Same reference identifier (`reference_identifier`) can have different fingerprints (different contexts).
-- The uniqueness is on identifier_fingerprint (PK), not reference identifier (`reference_identifier`).
-- Every fingerprint must have a reference identifier (`reference_identifier`) (no nulls).
--
-- Statement reference identifiers: s|p|o (pipe-delimited Fide IDs, 155 chars). Never stored here;
...
-- Statement reference identifier format: s|p|o pipe-delimited, no whitespace. Never in reference_identifiers.
...
-- ============================================================================
COMMENT ON TABLE reference_identifiers IS 'Fingerprint -> reference identifier lookup. Maps 36-char fingerprints to human-readable identifiers. Statement reference identifiers (s|p|o pipe-delimited) are derived, not stored.';
